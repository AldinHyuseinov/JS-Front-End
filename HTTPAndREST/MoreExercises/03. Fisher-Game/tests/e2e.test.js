...
describe("E2E tests", function () {
  ...
  describe("Authentication", () => {
    ...
    it("register makes correct API call", async () => {
      ...
      const [request] = await Promise.all([onRequest(), page.click("form >> text=Register")]);
    ...
    it("login makes correct API call", async () => {
      ...
      const [request] = await Promise.all([onRequest(), page.click("form >> text=Login")]);
  ...
  describe("CRUD", () => {
    ...
    it("create makes correct API call for logged in user", async () => {
      ...
      const [request] = await Promise.all([onRequest(), page.click("fieldset >> .add")]);
